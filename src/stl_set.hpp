...
/*   Updated: 2025/03/29 18:08:09 by pbremond         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class set
{
	private:
		...
		{
			private:
				// NOTE: This is needed so that the set class can access the
			...
			public:
				...
				__map_iterator(__s_node<Key> *node = NULL, bool goto_begin = false) : _node(node)
				{
	...
	public:
		explicit set(Compare const& comp = Compare(), Allocator const& alloc = Allocator());
