...
#define MULTIMAP_DEBUG_VERBOSE	false
...
class multimap
{
	private:
		...
		struct __s_node
		{
			...
			void	rotateLeft(__s_node<ValueType> **treeRoot)
			{
				...
				if (son == NULL)
					throw (std::logic_error("multimap: attempted to left-rotate with no right child"));
				this->right = son->left;
				if (this->right != NULL)
			...
			void	rotateRight(__s_node<ValueType> **treeRoot)
			{
				...
				if (son == NULL)
					throw (std::logic_error("multimap: attempted to right-rotate with no left child"));
				this->left = son->right;
				if (this->left != NULL)
		...
		{
			private:
				// NOTE: This is needed so that the multimap class can access the
...
#if MULTIMAP_DEBUG_VERBOSE == true
		static std::ostream&	logstream; // Defined in multimap.tpp
#endif
	...
	public:
		...
		class value_compare : public ft::binary_function<value_type, value_type, bool>
		{
			...
			public:
				using typename ft::binary_function<value_type, value_type, bool>::result_type;
				using typename ft::binary_function<value_type, value_type, bool>::first_argument_type;
				using typename ft::binary_function<value_type, value_type, bool>::second_argument_type;
	...
	public:
		...
		multimap&	operator=(multimap const& src);
	...
	private:
		...
		iterator	_correctInsertion(_Node_t *node, iterator const& retval);
...
namespace std
{
	...
	{
#if MULTIMAP_DEBUG_VERBOSE == true
			std::cout << _BLU "DEBUG: multimap: std::swap is specialized" RESET << std::endl;
#endif
		lhs.swap(rhs);
	}
}

#include "stl_multimap.tpp"
